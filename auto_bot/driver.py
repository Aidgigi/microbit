...
class Driver:
    ...
    def forward(self, time = 0, speed = 100):
        ...
        if time:
            if speed == 100:
                ...
                self.L_F.write_analog(math.floor((speed / 100) * self.analog_max))
                self.R_F.write_analog(math.floor((speed / 100) * self.analog_max))
                sleep(time)
                self.stop()
    ...
    def reverse(self, time = 0, speed = 100):
        if speed == 100:
            ...
            self.L_B.write_analog(math.floor((speed / 100) * self.analog_max))
            self.R_B.write_analog(math.floor((speed / 100) * self.analog_max))
            sleep(time)
            self.stop()
        ...
        if mode == 1:
            if direction == 1:
                if speed == 100:
                    ...
                    spe = math.floor((speed / 100) * self.analog_max)
